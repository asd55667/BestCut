...
/* 
  row
*/
export const searchMainIdx = (list: Track[], dx: number, idx: number) => {
  if (dx > 0) {
...
export const updateMainOrder = (list: Track[], dx: number, j: number, track?: Track) => {
  ...
  0 && swap<Track>(list, idx, j);
};

// non initial trak's marginLeft > 0
export const searchRowIdx = (list: Track[], dx: number, idx: number) => {
  let overlap = false;
...
export const updateOrder = (list: Track[], dx: number, j: number) => {
...
export const deleteTrack = (lists: Track[][], i: number, j: number, inMain: boolean) => {
  const list = lists[i];
